...
def normalize_coords(coords, h, w, no_shift=False):
    ...
    if no_shift:
        ...
        return coords / torch.tensor([w, h], device=coords.device) * 2 - 1.
...
def gen_grid(h, w, device, normalize=False, homogeneous=False):
    if normalize:
        ...
        lin_y = torch.arange(0, h, device=device) + 0.5
        lin_x = torch.arange(0, w, device=device) + 0.5
...
def gen_grid_np(h, w, normalize=False, homogeneous=False):
    if normalize:
        ...
        lin_y = np.arange(0, h) + 0.5
        lin_x = np.arange(0, w) + 0.5
...
def drawTrajectory(img, traj, radius=2, idx_vis=None, num_vis=200, mask=None):
    # traj: [Nt, Np, 2]
    traj = np.stack(traj, axis=0)
    num_pts = traj.shape[1]
    if idx_vis is None:
        if num_vis < num_pts:
            idx_vis = np.random.choice(num_pts, num_vis, replace=False)
        else:
    ...
    if mask is not None:
        ind = np.argsort(mask)[::-1]
        mask = mask[ind]
        traj = np.stack([x[ind] for x in traj])

    traj = traj.transpose(1, 0, 2)  # [Np, Nt, 2]

    ### color by angle
    for i, pt in enumerate(traj):
        coord_angle = np.arctan2(pt[0,1] - center[1], pt[0,0] - center[0])
        corr_color = np.int32(64 * coord_angle / np.pi) % 128
        color = tuple(colors[corr_color].tolist())

        # pt : [Nt,2]
        pt0 = pt[0]
        if (pt0[0] <= w - 1) and (pt0[0] >= 0) and (pt0[1] <= h - 1) and (pt0[1] >= 0):
            img = cv2.circle(img, (int(pt0[0]), int(pt0[1])), radius, color, -1, cv2.LINE_AA)
        img = cv2.polylines(img=img, 
                            pts=[pt.astype(np.int32)], 
                            isClosed=False, 
                            color=color, 
                            thickness=2)
    return img
...
def drawTrajectoryWithColor(img, traj, color_list, radius=2, idx_vis=None, num_vis=200, mask=None):
    # traj: [Nt, Np, 2]
    traj = np.stack(traj, axis=0)
    num_pts = traj.shape[1]
    num_t = traj.shape[0]
...
def draw_tracks_2d(
    ...
    # (H, W, 3).
    img_np = (img* 255.0).astype(np.uint8)
    # (time, point, 2) -> (point, time, 2)
    tracks_2d_np = tracks_2d.transpose(1, 0, 2)
...
def save_tensor_to_ply(tensor, fname):
    np_pcd = tensor.view(-1, 3).detach().cpu().numpy()
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(np_pcd)
    o3d.io.write_point_cloud(fname, pcd)
    
def depth_loss_dpt(pred_depth, gt_depth, weight=None, mask=None):
    """
    ...
    if mask is not None:
        pred_depth_n = pred_depth_n.view(-1, 1)[mask.view(-1)>0]
        gt_depth_n = gt_depth_n.view(-1, 1)[mask.view(-1)>0]
        pass
...
def compute_dynamic_position(static_pos, motion_params, detach=False, forward=True, label=None):
    if detach:
    ...
    if label is not None:
        trans_homo_select = compute_transform_with_kmeans_label(trans_homo.view(-1, 16), label).view(-1, 4, 4)
    else:
        trans_homo_select = trans_homo
    
    
    if forward is not True:
        final_trans_homo = torch.linalg.inv(trans_homo_select)
    else:
        final_trans_homo = trans_homo_select
    ...
    if final_trans_homo.shape[0] != homo_pos.shape[0]:
        raise ValueError('transformation shape is not equal to number of points')
    else:
        valid_trans = final_trans_homo
    
    dy_pos = torch.bmm(valid_trans, homo_pos.unsqueeze(-1)).squeeze(-1)
    # torch.einsum('ijk,kv->')
    final_pos = dy_pos[:, :3] / dy_pos[:, 3:]
    
    return final_pos

def compute_dynamic_rotation(cur_q, motion_params, detach=False, forward=True, label=None):
    ...
    if label is not None:
        dy_q_select = compute_transform_with_kmeans_label(dy_q, label)
    else:
        dy_q_select = dy_q
    
    # dynamic
    if cur_q.shape[0] != dy_q_select.shape[0]:
        raise ValueError('cur_q.shape != dy_q_select.shape')
    else:
        valid_q = dy_q_select
...
def connect_keypoints(image_1, image_2, kps_1, kps_2):
    ...
    for (x1, y1), (x2, y2) in zip(kps_1, kps_2):
        ...
        cmap_value = i / max(1, float(num_tracks - 1))
        color = tuple(np.array(cmap(cmap_value)[:3]) * 255)
        cv2.circle(output_image, point1, 1, color, 1)
        cv2.circle(output_image, point2, 1, color, 1)
        
        i += 1

    return output_image   

def compute_transform_with_kmeans_label(transforms, labels):
    '''
    transforms: [K, L] vector, K centroids, L vector length
    labels: [N, K], N points, K centroids, one hot vector
    '''
    broadcast_trans = torch.mm(labels.to(transforms), transforms)
    
    return broadcast_trans
